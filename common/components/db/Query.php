...
use Yii;
use common\components\base\Component;
...
class Query extends \yii\db\Query {
    ...
    public function where($condition, $params = []) {
        ...
        $tb = (strtok($this->from[0], " "));
        if ($tb) {
            $app = $tb . ".application_ID";
            $branch = $tb . ".branch_ID";
        } else {
            if (isset($this->modelClass)) {
                $app = call_user_func(array($this->modelClass, 'tableName')) . '.application_ID';
                $branch = call_user_func(array($this->modelClass, 'tableName')) . '.branch_ID';
            } else {
                $app = 'application_ID';
                $branch = 'branch_ID';
            }
        }
        $this->andWhere(["=", $app, \Yii::$app->session['app_ID']]);
        if (isset($this->modelClass)) {
            $model_class = $this->modelClass;
            if ($model_class != 'common\models\Branch' && !$consolidation) {
                $this->andWhere(["=", $branch, \Yii::$app->session['branch_ID']]);
            }
        } else {
            if (!$consolidation && Yii::$app->controller->id!='branch') {
                $this->andWhere(["=", $branch, \Yii::$app->session['branch_ID']]);
            }
        }
