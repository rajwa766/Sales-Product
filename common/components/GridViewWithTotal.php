 ...
 * @since 2.0
 */
class GridViewWithTotal extends BaseListView
{
    ...
    public function renderTableBody()
    {
        ...
        foreach ($models as $index => $model) {
            ...
            if($model->dr<$model->cr){
                $this->credit_total = $this->credit_total+($model->cr-$model->dr);
            }else{
                $this->debit_total = $this->debit_total+($model->dr-$model->cr);
            }
        ...
        $tr = "<tr class='total-row'><td colspan='".$this->colsize."'>Total</td><td>".number_format($this->debit_total,2)."</td><td>".number_format($this->credit_total,2)."</td></tr>";
        if (empty($rows)) {
