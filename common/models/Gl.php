...
class Gl extends \yii\db\ActiveRecord
{
    ...
    public static function get_filtered_result($user_id, $fromDate, $toDate, $account_type)
    {
        ...
        if (!isset($Role['super_admin'])) { // show only result for logged in user and not all payable/receivables
            if ($account_type == 1) {
                $gl->andWhere(['hit_user.id' => $logged_in_user]);
            } else {
                $gl->andWhere(['org_user.id' => $logged_in_user]);
            }
        }
